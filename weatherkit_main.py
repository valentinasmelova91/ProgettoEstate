...
#coordinates available in the past saved observations from all the sources
    def import_forecasts(self):
        ...
        # WeatherKit FORECAST
        try:
            wk_forecast = Forecasts_WeatherKit()
            wk_new_forecast = wk_forecast.export_forecasts(locations_list)
            os.chdir(oldpwd)
            os.chdir(os.getcwd() + '\Forecasts')
            now = datetime.datetime.utcnow().strftime("%Y%m%d%H%M%S")
            wk_new_forecast.to_csv(f'weatherkit_new_forecasts_full{now}.csv', index=False)
        except:
            print("WeatherKit Forecast was not reloaded")
